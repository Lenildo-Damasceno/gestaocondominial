 ...
 * lib/condominios.js
 * 
 * Dados mock e funções auxiliares para gerenciamento de condomínios
 * - Mantém dados em localStorage com chave: 'gestao-condominio:condominios'
 * - Dados base: 2 condomínios com contas, avisos, manutenções e assembleias
 * - Funções utilitárias para:
 *   - Listar, buscar e criar condomínios
 *   - Formatar datas, moedas e recorrências
 *   - Calcular urgências e resumos do dashboard
 *   - Gerenciar exclusão de condomínios (localStorage)
 * - Estrutura dados: contas (fixa/serviço), manutenções, avisos, assembleias
 */

const STORAGE_KEY = 'gestao-condominio:condominios'
export const CONDOMINIOS_CHANGE_EVENT = 'gestao-condominio:condominios-change'
...
function isManutencaoPlanejamento(manutencao) {
  return (
    String(manutencao?.titulo || '').toLowerCase().startsWith('planejar manutencao') &&
    !manutencao?.proximaData &&
    String(manutencao?.responsavel || '').toLowerCase() === 'definir fornecedor' &&
    String(manutencao?.status || '').toLowerCase() === 'pendente'
  )
}

function normalizarCondominios(condominios) {
  return condominios.map((condominio) => ({
    ...condominio,
    manutencoes: (condominio.manutencoes || []).filter(
      (manutencao) => !isManutencaoPlanejamento(manutencao)
    ),
  }))
}
...
export function listarCondominios() {
  // Sempre tenta carregar do localStorage primeiro, com fallback para dados base
  try {
    if (typeof window !== 'undefined') {
      ...
      if (salvo !== null) {
        ...
        if (Array.isArray(parsed)) {
          return normalizarCondominios(parsed)
        }
      }
    }
  } catch (error) {
    // Em caso de erro, continua com dados base
    console.warn('Erro ao carregar dados do localStorage:', error)
  }

  return normalizarCondominios(condominiosBase)
}
...
export function salvarCondominios(condominios) {
  ...
  window.dispatchEvent(new Event(CONDOMINIOS_CHANGE_EVENT))
}

export function escutarMudancasCondominios(callback) {
  if (!isClient()) {
    return () => {}
  }
  ...
  window.addEventListener(CONDOMINIOS_CHANGE_EVENT, handler)
  window.addEventListener('storage', handler)

  return () => {
    window.removeEventListener(CONDOMINIOS_CHANGE_EVENT, handler)
...
export function editarManutencaoDoCondominio(slug, id, dados) {
  return atualizarCondominio(slug, (condominio) => ({
    ...condominio,
    manutencoes: condominio.manutencoes.map((item) =>
...
export function excluirManutencaoDoCondominio(slug, id) {
  return atualizarCondominio(slug, (c) => ({
    ...c,
    manutencoes: c.manutencoes.filter((item) => item.id !== id),
  }))
}
...
export function excluirContaDoCondominio(slug, id) {
  return atualizarCondominio(slug, (c) => ({
    ...c,
    contas: c.contas.filter((item) => item.id !== id),
  }))
}
...
export function excluirAvisoDoCondominio(slug, id) {
  return atualizarCondominio(slug, (c) => ({
    ...c,
    avisos: c.avisos.filter((item) => item.id !== id),
  }))
}
...
export function excluirAssembleiaDoCondominio(slug, id) {
  return atualizarCondominio(slug, (c) => ({
    ...c,
    assembleias: c.assembleias.filter((item) => item.id !== id),
  }))
}
    ...
    manutencoes: c.manutencoes.map((item) =>
      item.id === manutencaoId
        ? {
            ...item,
            historico: [
              { id: criarId('hist'), data: registro.data, status: registro.status, observacao: registro.observacao || '', executor: registro.executor || '' },
              ...(item.historico || []),
            ],
          }
        : item
...
export function resumirDashboard(condominios) {
  const contas = condominios.flatMap((item) =>
    item.contas.map((conta) => ({
      ...conta, // Não normalizar para evitar cálculos dinâmicos
  ...
  return {
    ...
    manutencoesAlerta: manutencoes
      .filter((item) => STATUS_CRITICO.includes(item.status))
      .slice(0, 6),
...
export function formatarData(data) {
  ...
  try {
    // Garante formato consistente independente da localidade
...
export function normalizarConta(conta) {
  // Só calcula vencimento se não estiver definido ou se for conta recorrente
