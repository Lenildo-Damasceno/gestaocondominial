    ...
    <html lang="pt-BR" className="h-full antialiased" data-scroll-behavior="smooth">
      <body className="min-h-full flex flex-col">
        ...
        <header className="sticky top-0 z-50 flex items-center justify-between border-b border-white/8 bg-[#04101d]/90 px-5 py-3 backdrop-blur">
          ...
          <Link
            href="/dashboard"
            className="inline-flex items-center gap-2 rounded-full bg-gradient-to-r from-[#0f52ff] to-[#22d3ee] px-6 py-2.5 text-sm font-semibold text-white shadow-lg shadow-cyan-500/20 transition hover:shadow-xl hover:shadow-cyan-500/30 hover:scale-105 active:scale-95"
          >
        ...
        {/* Rodapé mobile fixo */}
        <footer className="fixed bottom-0 inset-x-0 z-[9999] flex sm:hidden items-center justify-around border-t border-white/10 bg-[#04101d]/95 backdrop-blur px-2 py-2">
          <Link href="/dashboard" className="flex flex-col items-center gap-1 px-4 py-2 text-white/80 active:scale-95 transition">
            <div className="flex h-9 w-9 items-center justify-center rounded-xl bg-gradient-to-br from-[#0f52ff] to-[#22d3ee]">
              <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 text-white" viewBox="0 0 24 24" fill="currentColor"><path d="M1 22V9l11-7 11 7v13H15v-7h-6v7H1z"/></svg>
            </div>
            <span className="text-xs font-semibold">Início</span>
          </Link>
          <Link href="/condominios" className="flex flex-col items-center gap-1 px-4 py-2 text-white/60 active:scale-95 transition hover:text-white/80">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 24 24" fill="currentColor"><path d="M17 11V3H7v4H3v14h8v-4h2v4h8V11h-4zm-8 4H7v-2h2v2zm0-4H7V9h2v2zm0-4H7V5h2v2zm4 8h-2v-2h2v2zm0-4h-2V9h2v2zm0-4h-2V5h2v2zm4 8h-2v-2h2v2zm0-4h-2V9h2v2z"/></svg>
            <span className="text-xs font-semibold">Condomínios</span>
          </Link>
          <Link href="/configuracoes" className="flex flex-col items-center gap-1 px-4 py-2 text-white/60 active:scale-95 transition hover:text-white/80">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" viewBox="0 0 24 24" fill="currentColor"><path d="M19.14 12.94c.04-.3.06-.61.06-.94s-.02-.64-.07-.94l2.03-1.58a.49.49 0 0 0 .12-.61l-1.92-3.32a.49.49 0 0 0-.59-.22l-2.39.96a7.01 7.01 0 0 0-1.62-.94l-.36-2.54a.484.484 0 0 0-.48-.41h-3.84c-.24 0-.43.17-.47.41l-.36 2.54c-.59.24-1.13.57-1.62.94l-2.39-.96a.47.47 0 0 0-.59.22L2.74 8.87a.47.47 0 0 0 .12.61l2.03 1.58c-.05.3-.07.62-.07.94s.02.64.07.94l-2.03 1.58a.47.47 0 0 0-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.37 1.03.7 1.62.94l.36 2.54c.05.24.24.41.48.41h3.84c.24 0 .44-.17.47-.41l.36-2.54c.59-.24 1.13-.57 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32a.47.47 0 0 0-.12-.61l-2.01-1.58zM12 15.6c-1.98 0-3.6-1.62-3.6-3.6s1.62-3.6 3.6-3.6 3.6 1.62 3.6 3.6-1.62 3.6-3.6 3.6z"/></svg>
